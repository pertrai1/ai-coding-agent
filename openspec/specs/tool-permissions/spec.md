...
The system SHALL assign default permission modes based on tool mutability. Read-only tools (`read_file`, `glob`, `grep`) MUST default to `"allow"`. Mutating tools (`write_file`, `edit_file`, `bash`) MUST default to `"prompt"`.
