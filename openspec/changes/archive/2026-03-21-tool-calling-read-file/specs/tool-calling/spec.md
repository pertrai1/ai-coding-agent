...
The system SHALL maintain a tool registry that maps tool names to their definitions and executor functions. Tools MUST be registered before the agent loop starts. The registry SHALL expose a method to look up a tool by name and a method to get all tool definitions for API requests.
