...
function classifyWriteError(error: unknown, filePath: string): ToolResult {
  if (error instanceof Error) {
    ...
    return { content: `Error writing file ${filePath}: ${error.message}`, isError: true };
  }

  return { content: `Error writing file ${filePath}: ${String(error)}`, isError: true };
}
...
async function execute(input: Record<string, unknown>): Promise<ToolResult> {
  ...
  try {
    ...
    return classifyWriteError(error, filePath);
  }
}
